 ...
 */
@Component
public class PetStatusFormatter implements Formatter<PetStatus> {

	private final PetService peService;

	@Autowired
	public PetStatusFormatter(PetService petService) {
		this.peService = petService;
	}

	@Override
	public String print(PetStatus petStatus, Locale locale) {
		return petStatus.getName();
	}

	@Override
	public PetStatus parse(String text, Locale locale) throws ParseException {
		Collection<PetStatus> findPetStatus = this.peService.findPetStatus();
		for (PetStatus status : findPetStatus) {
